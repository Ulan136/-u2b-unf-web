...
## 🗺️ Дорожная карта копирования УНФ (что готово ✅ / чего нет ⬜)
**Компания:** ✅ Организации · ⬜ Подразделения · ⬜ **Пользователи и вход в систему** · ⬜ Настройки учёта
**CRM:** ✅ Контрагенты · ⬜ Лиды/воронка · ⬜ Задачи/события/календарь · ⬜ Договоры
**Продажи:** ✅ Заказ покупателя · Отгрузка · Счёт(печать) · Оплата · ⬜ Возврат от покупателя · ⬜ Цены/прайс/скидки · ⬜ Розница ККМ/чеки/смены
**Закупки:** ✅ Заказ поставщику · Поступление · Оплата · ⬜ Возврат поставщику
**Склад:** ✅ Склады · Остатки · Перемещения · ⬜ Инвентаризация · ⬜ Оприходование/списание · ⬜ Комплектация · ⬜ Характеристики/партии/серийники · ⬜ Единицы (коэффициенты)
**Производство:** ✅ Спецификации · Производство(выпуск+себест) · ⬜ Заказ на производство · ⬜ Сдельные наряды
**Деньги:** ✅ Касса/банк · Приход/расход · Оплата по документу · ⬜ ПКО/РКО/платёжки с печатью · ⬜ Платёжный календарь · ⬜ Подотчётники · ⬜ Эквайринг
**Зарплата/Персонал:** ⬜ **Сотрудники** · ⬜ Начисление/выплата ЗП · ⬜ Табель/кадры
**Отчёты:** ✅ Остатки · Продажи · Закупки · Взаиморасчёты · Движение денег · Прибыль · ⬜ Оборачиваемость/ABC/план-факт
**Интеграции/Сервис:** ⬜ Синхрон с Юкан(частично заложен) · ⬜ Дашборд(в конце) · ⬜ PWA(в конце)

Приоритеты из пропущенного: 1) 🔐 Вход/пользователи (делаем сейчас) · 2) Возвраты · 3) Инвентаризация · 4) Розница/касса · 5) Сотрудники+ЗП · 6) Цены.
...
- ✅ Шаг 22 (2026-07-31): Документ **«Производство»**. Новое: таблицы `unf_productions`
    ...
    повтор и нехватка отклонены). tsc/build чисто. ЗАПУШЕНО и ЗАДЕПЛОЕНО (commit fc25a56), прод проверен.

### БЕЗОПАСНОСТЬ:
- ✅ Шаг 23 (2026-07-31): **Вход в систему (пользователи/сессии)**. Новое: таблица `unf_users`
    (username, password_hash, role, isActive) + db:push; `lib/session.ts` (HMAC-подпись куки через Web Crypto,
    edge-safe для middleware), `lib/password.ts` (scrypt, без зависимостей), `users.repo.ts`, `auth.service.ts`
    (register первого админа только при 0 юзеров, login); API `/api/auth/login|register|logout|me`;
    `src/middleware.ts` защищает всё кроме /login, /api/auth/*, /api/health (⚠️ если `AUTH_SECRET` НЕ задан —
    авторизация ВЫКЛЮЧЕНА, безопасный откат); страница `/login` (автоопределение: первый запуск→создание админа,
    иначе вход); `UserMenu` в шапке главной (имя + Выйти). Smoke-тест пройден (401 без входа, регистрация,
    доступ по сессии, выход, неверный пароль). tsc/build чисто.
    ⚠️ **Чтобы авторизация включилась на проде — добавить `AUTH_SECRET` в Vercel Environment Variables и передеплоить.**
    Значение AUTH_SECRET лежит в nextjs .env.local (локально); в git НЕ коммитим. Коммит готов, ждём «да» на push.
