...
function docNo(seq?: number) {
  return seq ? `ПН-${String(seq).padStart(5, "0")}` : "новый";
}
function today() {
...
export default function ReceiptsPage() {
  ...
  async function openEdit(id: string) {
    ...
    try {
      ...
      const json = await res.json();
      if (!json.ok) throw new Error(json.error ?? "Ошибка");
      const { receipt, items } = json.data;
      setEd({
        id: receipt.id,
        seq: receipt.seq,
        receiptDate: receipt.receiptDate ? String(receipt.receiptDate).slice(0, 10) : today(),
  ...
  async function save() {
    ...
    try {
      const payload = {
        receiptDate: ed.receiptDate || undefined,
      ...
      const res = await fetch(ed.id ? `/api/receipts/${ed.id}` : "/api/receipts", {
  ...
  async function archive() {
    if (!ed?.id) return;
    if (!confirm(`Пометить на удаление поступление ${docNo(ed.seq)}?`)) return;
    setBusy(true);
    try {
      const res = await fetch(`/api/receipts/${ed.id}`, { method: "DELETE" });
      const json = await res.json();
      if (!json.ok) throw new Error(json.error ?? "Ошибка");
      setEd(null);
      await loadList();
  ...
  async function receive() {
    if (!ed?.id) return;
    if (!confirm("Оприходовать поступление? Товар будет добавлен на склад (движение «Приход»).")) return;
    setBusy(true);
    setError(null);
    const rid = ed.id;
    try {
      const res = await fetch(`/api/receipts/${rid}/receive`, { method: "POST" });
      const json = await res.json();
      if (!json.ok) throw new Error(json.error ?? "Ошибка оприходования");
      await openEdit(rid);
      await loadList();
    } catch (e) {
      setError(e instanceof Error ? e.message : "Ошибка");
    ...
    try {
      const res = await fetch("/api/money/operations", {
        ...
        body: JSON.stringify({
          ...
          comment: data.comment || `Оплата поставщику по ${docNo(ed.seq)}`,
  ...
  return (
    <div className="min-h-screen">
      <header className="bg-yellow-400 text-gray-900 px-6 py-3 shadow flex items-center gap-4">
        ...
        <h1 className="text-lg font-semibold">Поступления (приход)</h1>
      </header>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-3 py-6 text-center text-gray-400">
                    Поступлений нет. Нажмите «+ Поступление».
                  </td>
                </tr>
              ) : (
                rows.map((o) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-gray-500">
                      {o.receiptDate ? String(o.receiptDate).slice(0, 10) : ""}
                    </td>
      ...
      {ed && payOpen && (
        <PaymentModal
          title={`Оплата поставщику по ${docNo(ed.seq)}`}
          kindLabel="Расход денег — оплата поставщику"
          accounts={accounts}
          defaultAmount={Math.max(0, edTotal - ed.paid)}
          defaultComment={`Оплата поставщику по ${docNo(ed.seq)}`}
          busy={busy}
          onPay={pay}
    ...
    <div className="fixed inset-0 bg-black/40 flex items-start justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-3xl mt-6 mb-10">
        <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
          <h3 className="font-semibold">Поступление {docNo(ed.seq)}</h3>
        ...
        <div className="p-4 space-y-3">
          {received && (
            <div className="bg-green-50 border border-green-200 text-green-800 text-sm rounded px-3 py-2">
              ✓ Оприходовано {String(ed.receivedAt).slice(0, 10)} — товар добавлен на склад.
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <label className="block">
              ...
              <input
                type="date"
                value={ed.receiptDate}
                onChange={(e) => setEd({ ...ed, receiptDate: e.target.value })}
                className="input"
              />
            </label>
            <label className="block">
            ...
            <label className="block">
              <span className="text-xs text-gray-500 mb-1 block">Склад приёмки</span>
              <select
          ...
          <div className="text-right space-y-0.5">
            ...
            {ed.id && (
              <div className="text-sm text-gray-500">
                Оплачено: <span className="text-red-600 font-mono">{money(ed.paid)}</span>
                {ed.paid < total && (
                  <>
                    {" "}· осталось{" "}
        ...
        <div className="px-4 py-3 border-t border-gray-200 flex items-center gap-2 flex-wrap">
          {ed.id && (
            <button
              ...
              title="Зарегистрировать оплату поставщику по этому поступлению"
            >
              💵 Оплата
            </button>
          )}
          ...
          {ed.id && !received && (
            <button
              ...
              title="Оприходовать товар на склад"
            >
              📥 Оприходовать
            </button>
          )}
